...
# def fetch_and_insert_news_for_countries(start_date, end_date, **kwargs):
...
#                             """, articles_to_insert)
#                             print(f"Inserted {len(articles)} articles into Snowflake table for {country}.")
#                         else:
#                             print(f"No valid articles found for date: {current_date.strftime('%Y-%m-%d')} and country: {country}.")
#                     except Exception as e:
#                         print(f"Error fetching data for date: {current_date.strftime('%Y-%m-%d')} and country: {country}. Error: {e}")
#                     current_date = next_day
#                     # Pause every 30th day to avoid overwhelming GDELT or Snowflake
#                     if current_date.day == 30:
#                         time.sleep(300)
#     except Exception as e:
#         print(f"Error fetching configuration or inserting data into Snowflake: {e}")
                ...
                while current_date <= end_date_dt:
                    ...
                    try:
                        ...
                        if not articles.empty:
                            ...
                            print(f"Inserted {len(articles)} articles into Snowflake table 'NEWS_DATA2' for {country}.")
                        else:
                            print(f"No valid articles found for date: {current_date.strftime('%Y-%m-%d')} and country: {country}.")
